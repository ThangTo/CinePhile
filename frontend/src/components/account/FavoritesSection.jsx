...
const FavoritesSection = ({ user }) => {
  ...
  const fetchData = async (page = 1) => {
    try {
      ...
      console.error("Error loading favorites data:", fetchError);
  ...
  const handleRemoveFavorite = async (e, movie) => {
    ...
    try {
      await userService.removeFromFavorites(targetMovieId);
      ...
      setError("Không thể xóa phim khỏi danh sách yêu thích. Vui lòng thử lại sau.");
    }
  };
  ...
  if (items.length === 0) {
    return (
      <div className="bg-account-bg-secondary rounded-2xl p-6 shadow-sm text-center">
        ...
        <h2 className="text-2xl font-semibold mb-2 text-account-text-primary">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <p className="text-account-text-secondary">
            {pagination.total || items.length} phim yêu thích
          </p>
        </div>
      </div>
