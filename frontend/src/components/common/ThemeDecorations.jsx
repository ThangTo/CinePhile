...
const ThemeDecorations = ({ theme }) => {
  ...
  useEffect(() => {
    ...
    // Bỏ qua decorations trên mobile để tránh crash
    if (isMobile && theme === "tet") {
      return;
    }
    ...
    try {
      if (theme === "tet") {
        ...
        // Throttle để không tạo quá nhiều cánh hoa (chỉ trên desktop)
        if (!isMobile) {
          mouseMoveHandlerRef.current = (e) => {
            try {
              ...
              console.error("Error creating mouse petal:", error);
            }
          };
        ...
        `;
          ...
          string.className = "tet-lantern-string";
          ...
          border-radius: 2px;
        `;
          // Thêm media query inline để ẩn trên mobile
      ...
      console.error("Error in ThemeDecorations:", error);
    }
