...
test('analyzeScenes delegates to the generic viral LLM provider config', async () => {
  ...
  assert.equal(calls[0].scope, 'VIRAL');
  assert.match(calls[0].messages[1].content, /00:00:10\.000 --> 00:00:42\.000/);
  assert.match(calls[0].messages[1].content, /Visual scene changes/);
